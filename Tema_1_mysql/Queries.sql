...
SELECT p.apellido1, p.apellido2, p.nombre, d.nombre as nombre_departamento FROM profesor p JOIN departamento d ON p.id_departamento = d.id ORDER BY p.apellido1, p.apellido2, p.nombre;
...
SELECT a.nombre, c.anyo_inicio, c.anyo_fin FROM alumno_se_matricula_asignatura am JOIN asignatura a ON am.id_asignatura = a.id JOIN curso_escolar c ON am.id_curso_escolar = c.id JOIN alumno al ON am.id_alumno = al.id WHERE al.nif = '26902806M'; 
...
SELECT DISTINCT d.nombre FROM departamento d JOIN profesor p ON d.id = p.id_departamento JOIN asignatura a ON p.id = a.id_profesor JOIN grado g ON a.id_grado = g.id WHERE g.nombre = 'Grado en Ingeniería Informática (Plan 2015)';


--
--9:Return a list with all/students who have enrolled in a subject during the 2018/2019 school year.
SELECT DISTINCT a.nombre, a.apellido1, a.apellido2 FROM alumno a JOIN alumno_se_matricula_asignatura am ON a.id = am.id_alumno JOIN curso_escolar c ON am.id_curso_escolar = c.id WHERE c.anyo_inicio = 2018  AND c.anyo_fin = 2019;
